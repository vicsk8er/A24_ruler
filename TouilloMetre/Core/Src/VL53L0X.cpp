...
bool VL53L0X::writeReg(uint8_t reg, uint8_t value) {
	...
	command2[0] = reg;
	command2[1] = value;
    if(_i2cHandler.write(_addr, command2, sizeof(command2)) != HAL_OK){
    	return false;
    }
	return true;
}
...
bool VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value){
	uint8_t command3[3];
	command3[0] = reg;
	command3[1] = value & 0xFF;
	command3[2] = value >> 8;

	if(_i2cHandler.write(_addr, command3, sizeof(command3)) != HAL_OK){
		return false;
	}
	return true;
}
...
// Write a 32-bit register
bool VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value){

	uint8_t command5[5];
	command5[0] = reg;
	command5[1] = value & 0xFF;
	command5[2] = value >> 8;
	command5[3] = value >> 16;
	command5[4] = value >> 24;

	if(_i2cHandler.write(_addr, command5, sizeof(command5)) != HAL_OK){
		return false;
	}
	return true;
}
...
uint8_t VL53L0X::readReg(uint8_t reg) {
	...
	uint8_t data1[1];
	command1[0] = reg;
	if(_i2cHandler.write(_addr, command1, sizeof(command1)) == HAL_OK){
		if (_i2cHandler.read(_addr, data1, sizeof(data1)) != HAL_OK) {
			return 0xFF; // Échec de lecture
		}
		return data1[0];
	}
	return 0xFF;//on retourne une valeur fuckall (la plus grande valeur de uint8_t)
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg) {
	...
	command1[0] = reg;
	if(_i2cHandler.write(_addr, command1, sizeof(command1)) == HAL_OK){
		if (_i2cHandler.read(_addr, data2, sizeof(data2)) != HAL_OK) {
			return 0xFFFF; // Échec de lecture
		}
		return (uint16_t)(data2[1] << 8 | data2[0]);
	}
	return 0xFFFF;//on retourne une valeur fuckall (la plus grande valeur de uint16_t)
}
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg) {
	...
	command1[0] = reg;
	if(_i2cHandler.write(_addr, command1, sizeof(command1)) == HAL_OK){
		if (_i2cHandler.read(_addr, data4, sizeof(data4)) != HAL_OK) {
			return 0xFFFFFFFF;// Échec de lecture
		}
		return (uint32_t)(data4[3] << 24 | data4[2] << 16 | data4[1] << 8 | data4[0]);
	}
	return 0xFFFFFFFF;//on retourne une valeur fuckall (la plus grande valeur de uint32_t)

}

// Write an arbitrary number of bytes from the given array to the sensor,
// starting at the given register
bool VL53L0X::writeMulti(uint8_t reg, uint8_t const *src, uint8_t count){
	uint8_t command7[7];
	command7[0] = reg;
	for(int i = 1; i < count + 1; i++){
		command7[i] = src[i - 1];
	}
    if(_i2cHandler.write(_addr, command7, count + 1) != HAL_OK){
    	return false;
    }
	return true;
}
...
bool VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count) {
	...
	command1[0] = reg;
	if(_i2cHandler.write(_addr, command1, sizeof(command1)) == HAL_OK){
		if (_i2cHandler.read(_addr, dst, count) != HAL_OK) {
...
// Public Methods //////////////////////////////////////////////////////////////
VL53L0X::VL53L0X(uint8_t addr, I2CHandler& i2cHandler): _addr(addr), _i2cHandler(i2cHandler),_name("VL53L0X"){
	initVL53L0X(true);//on dit qu'on utilise le mode 2v8
}


void VL53L0X::setAddress_VL53L0X(uint8_t new_addr) {
	if(writeReg(I2C_SLAVE_DEVICE_ADDRESS, (new_addr>>1) & 0x7F ) == HAL_OK){
		g_i2cAddr = new_addr;
	}

}

uint8_t VL53L0X::getAddress_VL53L0X() {
  return g_i2cAddr;
}
...
// mode.
bool VL53L0X::initVL53L0X(bool io_2v8){
...
uint16_t VL53L0X::getTimeout(void){
  return g_ioTimeout;
}
...
const char* VL53L0X::getFormattedMeasurement(){

	uint16_t distance = readRangeSingleMillimeters(&distanceStr);
	snprintf(_formattedBuffer, sizeof(_formattedBuffer), "Dist = %d mm", distance);
	return _formattedBuffer;

}

/*************************************************************************************************************************/
const char* VL53L0X::getSensorName(){
	return _name;
}
